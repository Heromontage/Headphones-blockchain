    ...
    this.program = new Program(this.gl, {
      ...
      fragment: `
        ...
        void main() {
          ...
          // Multiply color by alpha for premultiplied feel if needed, 
          // or just output raw texture color with SDF alpha
...
class App {
  ...
  createMedias(items: any[], bend = 1, textColor: string, borderRadius: number, font: string) {
    ...
    // To allow infinite scroll to work better, we duplicate the array
  ...
  // Public method to set index externally
  setIndex(index: number) {
    if (!this.medias || !this.medias[0]) return;
    const width = this.medias[0].width;
    // Map the conceptual index to the actual duplicated array
    // We just snap to the exact index in the array (modulo original length)
    // To find the closest match in the infinite ring:
    const originalLength = this.mediasImages.length / 2;
    const targetIdx = index % originalLength;
    this.scroll.target = targetIdx * width;
  ...
  onCheck() {
    ...
    // Calculate the actual logical index based on original array length
    const originalLength = this.mediasImages.length / 2;
    const sign = this.scroll.target < 0 ? -1 : 1;
    let logicalIndex = (sign * itemIndex) % originalLength;
    if (logicalIndex < 0) logicalIndex += originalLength;
    
    if (this.currentIndex !== logicalIndex) {
...
export default function CircularGallery({
  ...
  useEffect(() => {
    ...
    resolveFont(font, fontUrl).then((resolvedFont) => {
      ...
      if (activeIndex !== undefined && appRef.current) {
        appRef.current.setIndex(activeIndex);
      }
    });
  ...
  // Sync external activeIndex changes into the WebGL app
  useEffect(() => {
    if (appRef.current && activeIndex !== undefined && activeIndex !== appRef.current.currentIndex) {
      appRef.current.setIndex(activeIndex);
    }
  }, [activeIndex]);
